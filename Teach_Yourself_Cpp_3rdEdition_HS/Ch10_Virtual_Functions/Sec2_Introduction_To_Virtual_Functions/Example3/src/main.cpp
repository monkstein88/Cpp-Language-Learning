...
/*
	Example 3.: The next example shows how a virtual function can respond to random events that occur at run-time. This program
	selects between 'd_ob1' and 'd_ob2' based upon the value returned by the standard random number generator 'rand()'. Keep in
	mind that the version of 'func()' executed is resolved at run-time. (Indeed, it is impossible to resolve the calls to 'func()'
	at compile time.)
*/
