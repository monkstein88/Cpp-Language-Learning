...
/*
	Exercise 4.: Using a friend, show how to overload the '--' relative to the 'coord' class. Define both the prefix and postfix
	forms.
*/
...
public:
	...
	friend coord operator--(coord& ob); // This is unary '--' , which is for the pre-fix variant
	friend coord operator--(coord& ob, int notused); // This is unary '--'m which is for the post-fix variant
	friend coord operator/(coord ob1, coord ob2); // called when 'ob / ob'
	friend coord operator/(coord ob1, int i); // called when 'ob / int'
	friend coord operator/(int i, coord ob2); // called when 'int / ob'
	friend coord operator*(coord ob1, coord ob2); // called when 'ob * ob'
...
coord operator++(coord& ob, int notused){ // it's necessary to use reference parameter for unary operations
	...
	if(notused){ // By C++ specification, 'notused' is always supposed to be 0. It will not hold any other value in any case.
		cout << "\nError: Overloading postfix '++' - 'notused' is not 0!\n"; 	// So use this for signalling some kind of a compiler bug/error. This statement is useless, but it stops compilation errors
		exit(EXIT_FAILURE);
	}
	temp.x = ob.x;
	temp.y = ob.y;
...
// Overload the unary '--' using a friend function - call when the prefix variant is used
coord operator--(coord& ob){ // it's necessary to use reference parameter for unary operations
...
// Overload the unary '--' using a friend function - call when the postfix variant is used
coord operator--(coord& ob, int notused){  // it's necessary to use reference parameter for unary operations
	coord temp;
